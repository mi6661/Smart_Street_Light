...
public class LightRequest {
    ...
    /*
    * 通过id查寻路灯状态
    * 开发板通过各自不同的ID查询路灯的开关状态
    * 放回：off-路灯关闭 on-路灯开启 err-不存在该路灯，编号错误
    * */
    @GetMapping("/id")
    public String findById(@RequestParam int id){
        ...
        if(lightInfo == null){
            return "err";
        } else if(lightInfo.status.equals("on")){
            return "on";
        }else {
            return "off";
        }
    ...
    public ApiResonse<Boolean> liteUpdate(@RequestBody LightLite info){
